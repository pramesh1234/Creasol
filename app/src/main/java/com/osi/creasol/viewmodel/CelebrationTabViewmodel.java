...
public class CelebrationTabViewmodel {
    CelebrationTabFragment celebrationTabFragment;
    String schoolId, branchId, classId, divisionId, token;
    HashMap<String, String> header = new HashMap<>();
    public EventAdapter eventCelebrationAdapter;
    ArrayList<RowEventViewModel> viewModels;

    public CelebrationTabViewmodel(CelebrationTabFragment celebrationTabFragment) {
        this.celebrationTabFragment = celebrationTabFragment;
        getEventData();
    }

    private void getEventData() {
        EventSharedPreference eventSharedPreference = new EventSharedPreference(celebrationTabFragment.getContext());
        Session session = new Session(celebrationTabFragment.getContext());
        ...
        eventCelebrationAdapter = new EventAdapter(new ArrayList<RowEventViewModel>());
        EventApi eventApi = new EventApi(schoolId, branchId, classId, divisionId, header);
        eventApi.call(celebrationTabFragment.getContext(), new ApiCallback() {
            @Override
    ...
    private void fillEventData(EventData[] eventData) throws ParseException {
        ...
        for (int i = 0; i < eventData.length; i++) {
            if (eventData[i].getType() == 1) {
                DateFormat outputDayFormat = new SimpleDateFormat("EEE", Locale.US);
                ...
                RowEventViewModel viewModel = new RowEventViewModel(celebrationTabFragment, i, day, eventData[i].getType());
        ...
        eventCelebrationAdapter.addAll(viewModels);
    }
}
